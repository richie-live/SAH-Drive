...
def route_roadblock_correction2(
    ego_state: EgoState,
    ...
    :return: list of roadblock id's of corrected route
    """
    ending_block, ending_block_candidates = get_end_roadblock_candidates(
        mission_goal, map_api, route_roadblock_dict
    )
    ending_block_ids = [roadblock.id for roadblock in ending_block_candidates]
    ...
    # Fix ADD: when agent ends off-route
    if ending_block.id not in route_roadblock_ids:
        graph_search = BreadthFirstSearchRoadBlock(
            route_roadblock_ids[-1], map_api, forward_search=True
        )
        (path, path_id), path_found = graph_search.search(
            ending_block_ids, max_depth=search_depth_forward
        )

        if path_found:
            route_roadblocks[:len(route_roadblocks)] = path[1:]
            route_roadblock_ids[:len(route_roadblock_ids)] = path_id[1:]

        else:
            # Backward search to any route roadblock
            graph_search = BreadthFirstSearchRoadBlock(
                ending_block.id, map_api, forward_search=False
            )
            (path, path_id), path_found = graph_search.search(
                route_roadblock_ids[-3:], max_depth=search_depth_backward
            )

            if path_found:
                start_roadblock_idx = np.argmax(
                    np.array(route_roadblock_ids) == path_id[-1]
                )

                route_roadblocks = route_roadblocks[ : start_roadblock_idx - 1]
                route_roadblock_ids = route_roadblock_ids[ : start_roadblock_idx - 1]
...
def get_end_roadblock_candidates(
    ego_pose: StateSE2,
    map_api: AbstractMap,
    route_roadblocks_dict: Dict[str, RoadBlockGraphEdgeMapObject],
    heading_error_thresh: float = np.pi / 4,
