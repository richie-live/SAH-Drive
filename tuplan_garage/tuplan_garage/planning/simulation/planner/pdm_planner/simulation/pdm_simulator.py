...
class PDMSimulator:
    ...
    def simulate_proposals(
        ...
        self._tracker.update(proposal_states) # run update before track_trajectory !!!

        # state array representation for simulated vehicle states
        simulated_states = np.zeros(proposal_states.shape, dtype=np.float64)
        simulated_states[:, 0] = ego_state_to_state_array(initial_ego_state) # The reason why do I need to add 8 0s after there? stateindex 11, first 3 xytheta, other 8 are 0s
        ...
        current_iteration = SimulationIteration(current_time_point, 0) # timepoint，index
