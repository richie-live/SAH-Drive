...
class PLANTFPDMPlanner(AbstractPDMPlanner):
    """
    ...
    def __init__(
        ...
        super(PLANTFPDMPlanner, self).__init__(map_radius)
    ...
    def _get_closed_loop_trajectory(
        ...
        if self._diffusion_count == 0 and self._iteration % self._replan_freq == 0:
            ...
            if self._iteration % self._diffusion_freq == 0:
                ...
                imitation_planner = hydra.instantiate(config.imitation_planner)
                imitation_planner.initialize(self._initialization)
                # 检查实例化的对象
                local_trajectory = imitation_planner._planning_all(current_input)
                diffusion_proposals_local = local_trajectory
            ...
            #########################################select##############################
                ...
                # 定义优秀得分 s1
                s1 = 85  # 假设优秀得分为0.5，您可以根据实际情况设置

                # STDP参数设置
                tau_pos = 2  # 正STDP时间常数（LTP）
                tau_neg = 2  # 负STDP时间常数（LTD）
                A_plus = 1  # LTP增益
                A_minus = 0.5  # LTD衰减

                delta_t1 = 1/(best_pdm_score - s1)  # 系统1的得分差
                delta_t2 = 1/(best_diffusion_score - s1)  # 系统2的得分差
