...
class PDMGenerator:
    ...
    def generate_proposals(
        ...
        self._num_agent_ahead = -100 
        self._num_static_object_ahead = -100 
    ...
    def _update_leading_agents(
        ...
        if not update_leading_agent:
            ...
            for proposal_idx in lateral_batch_idcs:
                ...
                num_agent_ahead = 0
                num_static_object_ahead = 0
                for key in agents_ahead:
                    if key in self._observation.unique_objects:
                        agent_obj = self._observation.unique_objects[key]
                        if agent_obj.tracked_object_type in AGENT_TYPES:
                            num_agent_ahead = num_agent_ahead + 1
                        if agent_obj.tracked_object_type in STATIC_OBJECT_TYPES:
                            num_static_object_ahead = num_static_object_ahead + 1  

                if num_agent_ahead > self._num_agent_ahead:
                    self._num_agent_ahead = num_agent_ahead
                if num_static_object_ahead > self._num_static_object_ahead:
                    self._num_static_object_ahead = num_static_object_ahead
