...
class AbstractPDMPlanner(AbstractPlanner, ABC):
    ...
    def __init__(
        ...
        self._map_radius: int = map_radius  # type: ignore # [m]
    ...
    def _load_route_dicts(self, route_roadblock_ids: List[str]) -> None:
        ...
        for id_ in route_roadblock_ids:
            block = self._map_api.get_map_object(id_, SemanticMapLayer.ROADBLOCK) # type: ignore
            block = block or self._map_api.get_map_object( # type: ignore
    ...
    def _get_discrete_centerline(
        ...
        for lane in route_plan:
            centerline_discrete_path.extend(lane.baseline_path.discrete_path) # Add one Append, extend add a list
