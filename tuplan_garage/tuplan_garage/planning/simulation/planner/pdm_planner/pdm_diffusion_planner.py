...
class PDMDiffusionPlanner(AbstractPDMPlanner):
    ...
    def initialize(self, initialization) -> None:
        ...
        # Calculate the memory (bytes) occupied by model parameters
        total_memory = sum(p.numel() * p.element_size() for p in model.parameters())

        # Convert to KB/MB to display
        print(f"Total model memory: {total_memory / 1024:.2f} KB")
        print(f"Total model memory: {total_memory / (1024**2):.2f} MB")

        # Calculate and print the model parameter
